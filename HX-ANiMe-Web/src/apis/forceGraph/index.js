 ...
 * @param {*} fail 
 */
export const getApiKey = (userTableId, success, fail) => {
	Request.requestJson(Request.POST, currBaseUrl + "get-api-key?userTableId=" + userTableId)
		.then((data) => {
			if (data) {
				// 执行成功回调
				success(data.data);
 ...
 * @param fail 失败回调
 */
export const delUserTable = (userTableId, success, fail) => {
	Request.requestJson(
		Request.POST, 
		currBaseUrl + "remove-table?userTableId=" + userTableId, 
		{}
	).then((data) => {
			if (data) {
				// 执行成功回调
